...
# Haciendo el scalling de los datos
cols <- col
...
# Separando base de predictores (X) de la variable dependiente (Y = churn)
x_train <- train 
y_train <- train[, c("customer", "churn")]

#Haciendo la transformacion a matrix

x_train <- sparseMatrix(i =as.numeric(x_train$customer),j= as.numeric(x_train$customer))
y_train <- as.matrix(y_train)

# Haciendo el Lasso
lasso <- cv.gamlr(x_train, y_train, verb = T, family=c("gaussian","binomial","poisson"), lmr = .1)



# Código anterior, method = lasso no jala
numFolds = trainControl(method="cv", number = 10)
cpGrid = expand.grid(.cp = seq(0.01, 0.5, 0.01))
fit <- train(churn ~ . -customer, data = train, preProcess = "scale", method = "lasso", trControl = numFolds,tuneLength=1)
